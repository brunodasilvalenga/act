...
- For `ec2 ssh`: OpenSSH client (`ssh`), and either an SSH key already configured on the target instance, or `--push-key` to add your local public key to the target's `authorized_keys` via SSM Run Command (only needs the SSM Agent — no extra on-instance agent; the key persists until removed, and `--push-key` prints the exact command to remove it; not supported for Windows targets — use `ec2 rdp` instead); for `ec2 cp`: OpenSSH client (`scp`) and an SSH key configured on the target instance. `act doctor` checks for `ssh`/`scp` on PATH and warns (does not fail) if either is missing.
